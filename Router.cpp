...
int main() {
    ...
    // reading routes.txt
    ifstream fin;
    fin.open("/Users/Yu/Desktop/Network Programming/Router Project/Router Project/routes.txt");
    if (!fin.good()){
    ...
    fin.open("/Users/Yu/Desktop/Network Programming/Router Project/Router Project/arp.txt"); // open a file
    if (!fin.good()){
        fprintf(stderr, "File not found\n");
        return 1; // exit if file not found
    }
    ...
    // Parse pdu.txt
    fin.open("/Users/Yu/Desktop/Network Programming/Router Project/Router Project/PDU.txt"); // open a file
    if (!fin.good()){
        fprintf(stderr, "File not found\n");
        return 1; // exit if file not found
    }
    while (!fin.eof()) {
        ...
        if (ttl <= 1) {
            ...
            if (gateway == NULL) {
                ...
                cout << " via " << *gateway << "(" << *interface << "-" << getMAC("1.2.3.4", &arp_map) << " ttl " << ttl-1 << endl;
            }
        }
